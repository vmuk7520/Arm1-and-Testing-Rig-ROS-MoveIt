 ...
 * $ rostopic pub -1 /sync_set_position dynamixel_sdk_examples/SyncSetPosition "{id0: 0, id1: 1, id2: 2, position0: 0, position1: 0, position2: 1000}"
  ...
  dynamixel_sdk_examples::SyncGetPosition::Response & res)
{
  ...
  dxl_addparam_result = groupSyncRead.addParam((uint8_t)req.id0);
  if (dxl_addparam_result != true) {
    ROS_ERROR("Failed to addparam to groupSyncRead for Dynamixel ID %d", req.id0);
    return 0;
  }
  ...
  if (dxl_comm_result == COMM_SUCCESS) {
    position0 = groupSyncRead.getData((uint8_t)req.id0, ADDR_PRESENT_POSITION, 4);
...
void syncSetPositionCallback(const dynamixel_sdk_examples::SyncSetPosition::ConstPtr & msg)
{
  ...
  uint32_t position0 = (unsigned int)msg->position0; // Convert int32 -> uint32
  ...
  dxl_addparam_result = groupSyncWrite.addParam((uint8_t)msg->id0, param_goal_position0);
  if (dxl_addparam_result != true) {
    ROS_ERROR( "Failed to addparam to groupSyncWrite for Dynamixel ID %d", msg->id0);
  }
  ...
  if (dxl_comm_result == COMM_SUCCESS) {
    // ROS_INFO("setPosition : [ID:%d] [POSITION:%d]", msg->id0, msg->position0);
...
int main(int argc, char ** argv)
{
  ...
  dxl_comm_result = packetHandler->write1ByteTxRx(
    portHandler, DXL0_ID, ADDR_TORQUE_ENABLE, 1, &dxl_error);
  if (dxl_comm_result != COMM_SUCCESS) {
    ROS_ERROR("Failed to enable torque for Dynamixel ID %d", DXL0_ID);
    return -1;
  }
